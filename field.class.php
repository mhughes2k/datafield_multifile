...
class data_field_multifile extends data_field_base {
    var $type = 'multifile';
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        if ($formdata) {
            $fieldname = 'field_' . $this->field->id . '_multifile';
        ...
        // itemid element
        $html .= '<input type="hidden" name="field_'.$this->field->id.'_multifile" value="'.s($itemid).'" />';

        $options = new stdClass();
        $options->maxbytes = $this->field->param3;
        $options->maxfiles  = -1; // Change from file to unlimited.
    ...
    function display_browse_field($recordid, $template) {
        ...
        // we now need to explode $content by commas to get the list of files.
        $filenames = explode(",", $content->content);
        $files = [];
    ...
    function update_content($recordid, $value, $name='') {
        ...
        if (count($files) == 0) {
            ...
            // Store list of filenames as the content, comma separated
            $filenames = [];
            foreach($files as $file) {
                $filenames[] = $file->get_filename();
            }
            $content->content = implode(",", $filenames);
                //array_values($files)[0]->get_filename();
//            if (count($files) > 1) {
